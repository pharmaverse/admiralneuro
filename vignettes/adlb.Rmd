...
We advise you first consult the `{admiral}` [Creating a BDS Finding ADaM vignette](https://pharmaverse.github.io/admiral/main/articles/bds_finding.html).
...
adlb <- adlb %>%
  ## Join ADSL with lab data (need TRTSDT for ADY derivation) ----
  derive_vars_merged(
    dataset_add = adsl,
    new_vars = adsl_vars,
...
For deriving visits based on time-windows, see `{admiral}` [Visit and Period Variables](https://pharmaverse.github.io/admiral/main/articles/visits_periods.html).

## Derive Log Transformation of Biomarkers {#neuro_params}

In addition to the core ADLB variables, log transformation of fluid biomarker values are essential for further analysis and graphing as they are typically skewed.
...
The `{admiral}` [Creating a BDS Finding ADaM vignette](https://pharmaverse.github.io/admiral/main/articles/bds_finding.html) describes further steps, including how to calculate baseline and change from baseline variables, add analysis flags (e.g., `ANL01FL`), handle reference ranges, categorizations, and other common ADLB requirements.

# Example Scripts {#example}

ADaM | Sample Code
---- | --------------
ADLB | [ad_adlb.R](https://github.com/pharmaverse/admiralneuro/blob/main/inst/templates/ad_adlb.R){target="_blank"}
